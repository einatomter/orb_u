...
bool LocalMapping::InitializeVPBA(bool bFVPBA, int nMinKF, double minDepthDistance)
{
    ...
    {
        // old initialization
        optOK = CalculateScaleUW(mScale);
        optOK = Optimizer::ScaleRotationOptimizationUW(mpAtlas->GetCurrentMap(), mScale, mRwg, minDepthDistance, 0, false, true);
    }
...
bool LocalMapping::InitializeVIP(float priorG, float priorA, bool bFIBA, int nMinKF, double minDepthDistance)
{
    if (mbResetRequested)
        return false;
    ...
    {
        if (priorA!=0.f)
            Optimizer::FullVIPBA(mpAtlas->GetCurrentMap(), 100, false, mpCurrentKeyFrame->mnId, NULL, true, priorG, priorA);
        else
...
void LocalMapping::Run()
{
    ...
    while(1)
    {
        ...
        if(CheckNewKeyFrames() && !mbBadImu)
        {
            ...
            std::chrono::steady_clock::time_point profiling_start = std::chrono::steady_clock::now();
            ...
            if(!CheckNewKeyFrames() && !stopRequested())
            {
                ...
                if(mpAtlas->KeyFramesInMap()>2)
                {
                    if(mbInertial && mpCurrentKeyFrame->GetMap()->isImuInitialized())   // Inertial & initialized
                    {
                        ...
                        Optimizer::LocalInertialBA(mpCurrentKeyFrame, &mbAbortBA, mpCurrentKeyFrame->GetMap(),num_FixedKF_BA,num_OptKF_BA,num_MPs_BA,num_edges_BA, bLarge, !mpCurrentKeyFrame->GetMap()->GetIniertialBA2(), mbIsUW);
                    }
                    else if (mpAtlas->GetCurrentMap()->isDepthEnabledUW() && mbIsUW) // UW & initialized
                ...
                std::chrono::steady_clock::time_point profiling_end = std::chrono::steady_clock::now();
                // convert time to double in ms
                double profiling_time = std::chrono::duration_cast<std::chrono::duration<double, std::milli>>(profiling_end - profiling_start).count();
                mpROSPublisher->PublishTimeMS(profiling_time);
            }
