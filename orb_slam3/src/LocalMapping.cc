...
void LocalMapping::InitializeInertialUW(float priorG, float priorA, bool bFIBA)
{
    if (mbResetRequested)
        return;
        ...
        if(CheckNewKeyFrames() && !mbBadImu)
        {
            ...
            if(!CheckNewKeyFrames() && !stopRequested())
            {
                if(mpAtlas->KeyFramesInMap()>2)
                {
                    ...
                    {
                        if(mpCurrentKeyFrame->GetMap()->isScaleUWInitialized() && mbIsUW)   // UW
                            Optimizer::LocalBAUW(mpCurrentKeyFrame,&mbAbortBA, mpCurrentKeyFrame->GetMap(),num_FixedKF_BA,num_OptKF_BA,num_MPs_BA,num_edges_BA);
                        else
                ...
                {
                    // cout << "attempting IMU initialization" << endl;
                    if (mbMonocular)
