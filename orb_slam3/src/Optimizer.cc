...
int Optimizer::PoseOptimizationUW(Frame *pFrame)
{
    ...
    for(size_t it=0; it<4; it++)
    {
        ...
        // chi2 is mahalanobis distance
        // const float depthChi2 = eDepth->chi2();
        // std::cout << "chi2 depth:\t" << depthChi2 << std::endl;
...
void Optimizer::LocalBAUW(KeyFrame *pKF, bool* pbStopFlag, Map* pMap, int& num_fixedKF, int& num_OptKF, int& num_MPs, int& num_edges)
{
    ...
    for(list<MapPoint*>::iterator lit=lLocalMapPoints.begin(), lend=lLocalMapPoints.end(); lit!=lend; lit++)
    {
        ...
        for(map<KeyFrame*,tuple<int,int>>::const_iterator mit=observations.begin(), mend=observations.end(); mit!=mend; mit++)
        {
            ...
            if(!pKFi->isBad() && pKFi->GetMap() == pCurrentMap)
            {
                ...
                // Set Depth edge connected to pose
                UW::EdgeDepth2* eDepth = new UW::EdgeDepth2;
                eDepth->setVertex(0, optimizer.vertex(pKF->mnId));
...
void Optimizer::BAUW(const vector<KeyFrame *> &vpKFs, const vector<MapPoint *> &vpMP,
...
void Optimizer::GlobalBundleAdjustemnt(Map* pMap, int nIterations, bool* pbStopFlag, const unsigned long nLoopKF, const bool bRobust)
{
    ...
    if(pMap->isScaleUWInitialized())
        BAUW(vpKFs,vpMP,nIterations,pbStopFlag, nLoopKF, bRobust);
    else
