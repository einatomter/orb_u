...
bool Optimizer::ScaleOptimizationUW(Map *pMap, double &scale, Eigen::Matrix3d &Rwg, float minDepthDistance, int minKFs, bool setRwgFixed, bool setScaleFixed)
{
    ...
    VertexUWScale* VS = new VertexUWScale(scale);
    VS->setId(maxKFid + 1);
...
bool Optimizer::UWBA(Map* pMap, double &scale, Eigen::Matrix3d &Rwg, int nIterations, bool* pbStopFlag, const unsigned long nLoopKF, const bool bRobust, bool setRwgFixed, bool setScaleFixed, int minKFs, double minDepthDistance)
{
    ...
    const int gDirOffset = 1;
    const int scaleOffset = 2;
    const int mapPointOffset = 3;
    ...
    // Set scale and gravity vertices
    VertexUWScale* VS = new VertexUWScale(scale);
    VS->setId(maxKFid + scaleOffset);
    VS->setFixed(setScaleFixed);
    optimizer.addVertex(VS);
    VertexGDir* VGDir = new VertexGDir(Rwg);
    VGDir->setId(maxKFid + gDirOffset);
    VGDir->setFixed(setRwgFixed);
    optimizer.addVertex(VGDir);
    ...
    for(size_t i=0; i<vpKFs.size(); i++)
    {
        ...
        eDepth->setVertex(1, dynamic_cast<g2o::OptimizableGraph::Vertex*>(optimizer.vertex(maxKFid + scaleOffset)));
        eDepth->setVertex(2, dynamic_cast<g2o::OptimizableGraph::Vertex*>(optimizer.vertex(maxKFid + gDirOffset)));
    ...
    for(size_t i=0; i<vpMP.size(); i++)
    {
        ...
        const int id = pMP->mnId+maxKFid+mapPointOffset;
        vPoint->setId(id);
    ...
    for(size_t i=0; i<vpMP.size(); i++)
    {
        ...
        if(pMP->isBad())
            continue;
        g2o::VertexSBAPointXYZ* vPoint = static_cast<g2o::VertexSBAPointXYZ*>(optimizer.vertex(pMP->mnId+maxKFid+mapPointOffset));
...
int Optimizer::PoseOptimizationUW(Frame *pFrame, Frame *pFramePrev, bool isUW)
{
...

    {
        ...
        g2o::HyperGraph::Vertex* VP2 =  optimizer.vertex(0);

        if(!VP1 || !VP2)
        {
            cout << "Error " << VP1 << ", "<< VP2 << endl;
        }
...
bool Optimizer::ScaleRotationOptimizationUW(Map *pMap, double &scale, Eigen::Matrix3d &Rwg, float minDepthDistance, int minKFs, bool setRwgFixed, bool setScaleFixed)
{
    ...
    // Set edges
    for(size_t i=1; i<vpValidKFs.size(); i++)
    {
        KeyFrame* pPrevKFi = vpValidKFs[i-1];
        KeyFrame* pKFi = vpValidKFs[i];

        g2o::HyperGraph::Vertex* VP1 = optimizer.vertex(pPrevKFi->mnId);
        g2o::HyperGraph::Vertex* VP2 =  optimizer.vertex(pKFi->mnId);
        g2o::HyperGraph::Vertex* VGDir = optimizer.vertex(maxKFid + 1);
        g2o::HyperGraph::Vertex* VS = optimizer.vertex(maxKFid + 2);

        if(!VP1 || !VP2 || !VGDir || !VS)
        {
            cout << "Error " << VP1 << ", "<< VP2 << ", "<< VGDir << ", "<< VS << endl;
            continue;
        }
        ...
        Eigen::Vector2d depthMeasurement;
        depthMeasurement << pKFi->mPressureMeas.relativeDepthHeight() - pPrevKFi->mPressureMeas.relativeDepthHeight(), pKFi->mPressureMeas.relativeDepthHeight();
        eDepth->setMeasurement(depthMeasurement);
        eDepth->setDepthAxis(pKFi->mPressureMeas.depthAxis);  // TODO: declare as const at start of function?
...
bool Optimizer::FullVPBA(Map* pMap, double &scale, Eigen::Matrix3d &Rwg, int nIterations, bool* pbStopFlag, const unsigned long nLoopKF, const bool bRobust, bool setRwgFixed, bool setScaleFixed, int minKFs, double minDepthDistance)
{
    ...
    for(size_t i=1; i<vpValidKFs.size(); i++)
    {
        ...
        g2o::HyperGraph::Vertex* VP1 = optimizer.vertex(pPrevKFi->mnId);
        g2o::HyperGraph::Vertex* VP2 =  optimizer.vertex(pKFi->mnId);
        g2o::HyperGraph::Vertex* VGDir = optimizer.vertex(maxKFid + gDirOffset);
        g2o::HyperGraph::Vertex* VS = optimizer.vertex(maxKFid + scaleOffset);
...
void Optimizer::VIPOptimizationUW(Map *pMap, Eigen::Matrix3d &Rwg, double &scale, Eigen::Vector3d &bg, Eigen::Vector3d &ba, bool bMono, Eigen::MatrixXd  &covInertial, bool bFixedVel, bool bGauss, float priorG, float priorA, bool first)
{
    ...
    for(size_t i=0;i<vpKFs.size();i++)
    {
        ...
        if(pKFi->mPrevKF && pKFi->mnId<=maxKFid)
        {
            ...
            eDepth->setMeasurement(depthMeasurement);
            eDepth->setDepthAxis(Eigen::Vector3d(0.f, 0.f, -1.f));  // TODO: declare as const at start of function?
...
void Optimizer::FullVIPBA(Map *pMap, int its, const bool bFixLocal, const long unsigned int nLoopId, bool *pbStopFlag, bool bInit, float priorG, float priorA, Eigen::VectorXd *vSingVal, bool *bHess)
{
    long unsigned int maxKFid = pMap->GetMaxKFid();
    const vector<KeyFrame*> vpKFs = pMap->GetAllKeyFrames();
...
void Optimizer::GlobalBundleAdjustemnt(Map* pMap, int nIterations, bool* pbStopFlag, const unsigned long nLoopKF, const bool bRobust, bool isUW)
{
    vector<KeyFrame*> vpKFs = pMap->GetAllKeyFrames();
    vector<MapPoint*> vpMP = pMap->GetAllMapPoints();
    BundleAdjustment(vpKFs,vpMP,nIterations,pbStopFlag, nLoopKF, bRobust, isUW);
}


void Optimizer::BundleAdjustment(const vector<KeyFrame *> &vpKFs, const vector<MapPoint *> &vpMP,
                                 int nIterations, bool* pbStopFlag, const unsigned long nLoopKF, const bool bRobust, bool isUW)
{
    vector<bool> vbNotIncludedMP;
    vbNotIncludedMP.resize(vpMP.size());
...
int Optimizer::PoseOptimization(Frame *pFrame, bool isUW)
{
    g2o::SparseOptimizer optimizer;
    g2o::BlockSolver_6_3::LinearSolverType * linearSolver;
...
void Optimizer::LocalBundleAdjustment(KeyFrame *pKF, bool* pbStopFlag, Map* pMap, int& num_fixedKF, int& num_OptKF, int& num_MPs, int& num_edges, bool isUW)
{
...
void Optimizer::LocalInertialBA(KeyFrame *pKF, bool *pbStopFlag, Map *pMap, int& num_fixedKF, int& num_OptKF, int& num_MPs, int& num_edges, bool bLarge, bool bRecInit, bool bUW)
{
    Map* pCurrentMap = pKF->GetMap();
...
int Optimizer::PoseInertialOptimizationLastKeyFrame(Frame *pFrame, bool bRecInit, bool bUW)
{
    g2o::SparseOptimizer optimizer;
    g2o::BlockSolverX::LinearSolverType * linearSolver;
...
int Optimizer::PoseInertialOptimizationLastFrame(Frame *pFrame, bool bRecInit, bool bUW)
{
    g2o::SparseOptimizer optimizer;
    g2o::BlockSolverX::LinearSolverType * linearSolver;
